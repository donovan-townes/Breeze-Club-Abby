...
Abby issue backlog organized by priority. Issues #1 and #2 remain for historical
completeness. See [docs/architecture/ROADMAP.md](docs/architecture/ROADMAP.md) for
phase sequencing and [docs/architecture/ABBY_ROLE_AND_MODES.md](docs/architecture/ABBY_ROLE_AND_MODES.md)
for the portal posture.
...
**Status**: COMPLETED — RAG infrastructure ready, text preparation automated, Discord UI complete

- Scope: finish RAG retrieval + prompt injection with safe fallbacks + document management
- Acceptance:
  - ✅ Automatic text preparation (markdown stripping, emoji removal, canonical formatting)
  - ✅ `/rag ingest` with modal UI for clean document submission
  - ✅ `/rag list`, `/rag delete` for document management
  - ✅ Guild-scoped document storage and retrieval
  - ✅ TDOS-ready architecture (agent-agnostic, portable)
  - ⏳ Chatbot prompt injection (next step — wire RAG context into responses)
  - ⏳ Graceful fallback when RAG unavailable
  - ⏳ Configurable relevance threshold
- Implementation:
  - Module: [prepare.py](abby_core/rag/prepare.py) — text cleaning and canonical formatting
  - Handler: [handler.py](abby_core/rag/handler.py) — ingest/query/list/delete functions
  - Discord: [rag.py](abby_adapters/discord/cogs/admin/rag.py) — modal UI, unified command
  - Docs: [RAG_INGESTION_GUIDE.md](docs/features/RAG_INGESTION_GUIDE.md), [RAG_PREPARE_API.md](docs/api-reference/RAG_PREPARE_API.md)
  - Summary: [RAG_REFACTOR_SUMMARY.md](docs/architecture/RAG_REFACTOR_SUMMARY.md)
- Next Steps:
  - Wire RAG context into chatbot prompt (retrieve → inject → respond)
  - Add fallback logic when RAG unavailable or no results
  - Tune relevance threshold and top_k settings
- References: [abby_adapters/discord/cogs/creative/chatbot.py](abby_adapters/discord/cogs/creative/chatbot.py), [abby_core/rag/handler.py](abby_core/rag/handler.py), [abby_core/rag/prepare.py](abby_core/rag/prepare.py)
