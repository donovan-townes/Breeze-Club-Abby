...
from abby_adapters.discord.cogs.Twitch.twitch import get_user_twitch_handle, is_user_live
import os
...
# Helper to safely parse env vars as int
def safe_int_env(key, default='0'):
    value = os.getenv(key, default)
    try:
        return int(value) if value and value.strip() else 0
    except ValueError:
        logger.warning(f"Invalid integer for {key}: {value}, using 0")
        return 0

TEST_CHANNEL = safe_int_env('TWITCH_TEST_CHANNEL_ID') or None
BREEZE_TV = safe_int_env('TWITCH_NOTIFY_CHANNEL_ID') or None
Z8PHYR_FAM = safe_int_env('TWITCH_ROLE_Z8PHYR_FAM') or None
