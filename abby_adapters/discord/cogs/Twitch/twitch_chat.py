...
TWITCH_CLIENT_ID = os.getenv("TWITCH_CLIENT_ID")  # Required for twitchio v2+
TWITCH_CLIENT_SECRET = os.getenv("TWITCH_CLIENT_SECRET")  # Required for twitchio v2+
TWITCH_BOT_ID = os.getenv("TWITCH_BOT_ID")  # Required for twitchio v2+
...
class TwitchChatCog(discord_commands.Cog):
    ...
    class TwitchBotWrapper(twitch_commands.Bot):
        ...
        def __init__(self, parent_cog):
            # twitchio v2+ requires client_id, client_secret, and bot_id
            if not all([TWITCH_CLIENT_ID, TWITCH_CLIENT_SECRET, TWITCH_BOT_ID]):
                raise ValueError("TWITCH_CLIENT_ID, TWITCH_CLIENT_SECRET, and TWITCH_BOT_ID must be set in .env for twitchio v2+")
