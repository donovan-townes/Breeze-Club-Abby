...
from abby_core.utils.mongo_db import connect_to_mongodb, get_tenant_id
...
def get_xp_collection():
    """Get the XP collection from MongoDB."""
...
def get_xp(user_id):
    ...
    return xp_collection.find_one({"tenant_id": tenant_id, "user_id": user_id})


def add_xp(user_id, amount, reason="manual"):
    """Add XP to user (can be negative)."""
    user_id = str(user_id)
    xp_collection = get_xp_collection()
    tenant_id = get_tenant_id()
