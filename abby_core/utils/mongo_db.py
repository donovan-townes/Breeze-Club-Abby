...
import pymongo
import abby_core.utils.bdcrypt as bdcrypt
...
def get_tenant_id(user_id: int = None) -> str:
    """Get tenant ID for a user (uses user_id as tenant). If no user_id provided, returns default 'default'."""
    return str(user_id) if user_id else "default"


def get_sessions_collection(user_id: int = None):
    """Get the chat sessions collection. If user_id provided, returns user-specific collection."""
    client = connect_to_mongodb()
    if user_id:
        db = client[f"User_{user_id}"]
        return db["Chat Sessions"]
    else:
        # Global sessions collection (for backward compatibility)
        db = client["Abby_Sessions"]
        return db["Chat Sessions"]


def create_session(user_id: int, session_id: str, channel_id: int = None):
    """Create a new chat session."""
    try:
        collection = get_sessions_collection(user_id)
        session_doc = {
            "user_id": user_id,
            "session_id": session_id,
...
def append_session_message(user_id: int, session_id: str, role: str, content: str):
    ...
    try:
        collection = get_sessions_collection(user_id)
        encrypted_content = bdcrypt.encrypt(content, user_id)
...
def close_session(user_id: int, session_id: str, summary: str = None):
    """Close a chat session and optionally add a summary."""
    try:
        collection = get_sessions_collection(user_id)
        update_doc = {
...
def get_rag_documents_collection():
    """Get the RAG documents metadata collection."""
    client = connect_to_mongodb()
    db = client["Abby_RAG"]
