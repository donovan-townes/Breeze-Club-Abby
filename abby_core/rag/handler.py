...
def ingest(
    ...
    """Ingest a document into Chroma and MongoDB rag_documents with user/guild scoping."""
    tags = tags or []
...
def query(text: str, user_id: Optional[str] = None, guild_id: Optional[str] = None, top_k: int = 3) -> Dict[str, Any]:
    """Query RAG documents scoped by user_id and/or guild_id."""
    try:
        embedder = Embeddings()
        chroma = ChromaClient()
    except (EmbeddingError, ChromaUnavailable) as exc:
    ...
    # Filter by user_id/guild_id in metadata
    filtered = []
    ...
    for doc_id, meta, doc in zip(ids, metadatas, documents):
        if not meta:
            continue
        # Match if user_id/guild_id match (or if both are None for global docs)
