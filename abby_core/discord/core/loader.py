...
if HAS_CONFIG and BotConfig:
    try:
        _config = BotConfig.from_env()  # type: ignore[attr-defined]
...
class CommandHandler(commands.Cog):
    ...
    async def load_commands(self):  
        ...
        cog_summary_item = next(
            (item for item in reversed(loaded_items) if item[0] == "🐰" and item[1] == "Cogs"),
            None,
        )
        total_cogs = cog_summary_item[3] if cog_summary_item else 0
        total_commands = sum(int(item[3]) for item in loaded_items if item[1] == "Commands" and item[2] == "Loaded")
        failed = sum(1 for item in loaded_items if item[0] == "❌")
        
        # Group by category for cleaner display (exclude summary rows)
        categories = {}
        for item in loaded_items:
            if item[0] == "✅" and item[1] not in ["Commands", "Handlers", "Cogs"]:
                category = item[1]
                categories[category] = categories.get(category, 0) + 1
        
        # Log clean summary
        self.logger.info(f"[🐰] Loaded {total_cogs} cogs across {len(categories)} categories" + (f" ({total_commands} commands)" if total_commands > 0 else ""))

        # Persist counts for final startup summary
        self.last_load_summary = {
            "cogs": total_cogs,
            "commands": total_commands,
            "categories": len(categories),
        }
        self.bot.startup_cog_count = total_cogs
        self.bot.startup_command_count = total_commands
        
        # Show failed items prominently at startup (not just a count)
        if failed > 0:
            self.logger.warning(f"[⚠️] {failed} item(s) failed to load:")
            for item in loaded_items:
                if item[0] == "❌":
                    category, name, error = item[1], item[2], item[3]
                    # Truncate long error messages for readability
                    error_short = error[:100] + "..." if len(error) > 100 else error
                    self.logger.warning(f"  ❌ {category}/{name}: {error_short}")
        
        # Category breakdown (DEBUG level)
        for category, count in sorted(categories.items()):
            self.logger.debug(f"  • {category}: {count} cog(s)")
        
        # Detailed table (DEBUG level only)
    ...
    async def load_command_files(self, bot, loaded_items):
        ...
        loaded_commands = 0
        commands_dir = str(WORKING_DIRECTORY / 'abby_core' / 'discord' / 'commands')
        adapters_root = str(WORKING_DIRECTORY / 'abby_core')
        ...
        for root, dirs, files in os.walk(commands_dir):
            for file_name in files:
                if file_name.endswith(".py") and file_name != "__init__.py":
                    ...
                    module_path = "abby_core." + rel_path.replace(os.sep, ".")
    ...
    async def load_handler_files(self, bot, loaded_items):
        ...
        loaded_handlers = 0
        handlers_dir = str(WORKING_DIRECTORY / 'abby_core' / 'discord' / 'handlers')
        adapters_root = str(WORKING_DIRECTORY / 'abby_core')
        ...
        for file_name in os.listdir(handlers_dir):
            if file_name.endswith(".py") and file_name not in ["__init__.py", "command_loader.py"]:
                module_name = file_name[:-3]
                module_path = f"abby_core.discord.handlers.{module_name}"
    ...
    async def load_cog_files(self, loaded_items):
        ...
        main_directory = str(WORKING_DIRECTORY / 'abby_core' / 'discord' / 'cogs')
        adapters_root = str(WORKING_DIRECTORY / 'abby_core')
        cogs = []
            ...
            if root != main_directory:
                for file in files:
                    if file.endswith(".py") and file != "__init__.py":
                        ...
                        module_path = "abby_core." + rel_path[:-3].replace(os.sep, ".")

                        # Deprecated: Discord scheduler cog replaced by platform SchedulerService
                        if module_path == "abby_core.discord.cogs.system.scheduler":
                            self.logger.warning(
                                "[🐰] Skipping deprecated Scheduler cog (platform SchedulerService is canonical)"
                            )
                            loaded_items.append(("⚠️", category, item_name, "Skipped (deprecated: use SchedulerService)"))
                            continue
                        
                        try:
                            # Only log individual cog loads if TRACE mode enabled
                            if os.getenv("TRACE", "").lower() in ("1", "true", "yes"):
                                self.logger.debug(f"[🐰] Loading {category}/{item_name}...")
                            module = importlib.import_module(module_path)
