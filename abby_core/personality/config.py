...
class PersonaFileConfig:
    ...
    def __init__(self, persona: str = "bunny", config_dir: Optional[Path] = None):
        ...
        logger.info(f"[🎭] Persona file config initialized for '{persona}' from {self.config_dir}")
    
    @property
    def summon_words(self) -> List[str]:
        """Get list of words/phrases that summon Abby."""
        if self._summon_words is None:
            self._summon_words = self._load_json_file("summon.json", "summon_words", []) or []
        assert self._summon_words is not None
    ...
    def dismiss_words(self) -> List[str]:
        ...
        if self._dismiss_words is None:
            self._dismiss_words = self._load_json_file("dismiss.json", "dismiss_words", []) or []
        assert self._dismiss_words is not None
    ...
    def response_patterns(self) -> Dict[str, Any]:
        ...
        assert self._response_patterns is not None
...
_personality_configs: Dict[str, PersonaFileConfig] = {}


def get_personality_config(persona: str = "bunny") -> PersonaFileConfig:
    """
    ...
    Returns:
        PersonaFileConfig instance for the specified persona
    """
    global _personality_configs
    if persona not in _personality_configs:
        _personality_configs[persona] = PersonaFileConfig(persona=persona)
...
# Backwards compatibility alias to reduce breakage while phasing out the old name
PersonalityConfig = PersonaFileConfig
